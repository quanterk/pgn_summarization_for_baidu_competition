...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.embedding.weight.data.copy_(torch.from_numpy(self.load_embedding_matrix()))
        print('加载词向量完成')
        #词向量一起进行训练
    ...
    def load_embedding_matrix(self, filepath=config.embedding_matrix_path, max_vocab_size=config.vocab_size):
        """
        加载 embedding_matrix_path
        """
        embedding_matrix = np.load(filepath + '.npy')
        print('loaded word vector############')
