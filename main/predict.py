...
def predict(txt, i, num_arr):
    global all, cur, vocab, beam_processor
    batch = build_batch_by_article(txt, vocab)
    out = beam_processor.decode(batch)
    num_arr[0] -= 1
    print(i, ':', num_arr)
    return out
...
def parallelize_func(data):
    ...
    num = df.shape[0]
    num_arr = [num]
    print('num_arr:', i, num_arr)
    df['Prediction'] = df['merged'].apply(lambda x: predict(x, i, num_arr))
    return df
...
if __name__ == '__main__':
    ...
    out_df = pd.DataFrame()
    out_df['QID'] = test_df['QID'].values
    out_df['Prediction'] = test_df['Prediction'].values
